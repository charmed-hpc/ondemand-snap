...
--- Wrapper for `nginx_stage` configuration.
---@class NginxStage
---@field private file string Location of _nginx-stage.yaml_ configuration file.
local NginxStage = {}

--- Create a new `NginxStage` configuration wrapper.
---@return NginxStage
function NginxStage.new()
  local self = {}
  setmetatable(self, { __index = NginxStage })
  self.file = snap.paths.common .. "/etc/ondemand/nginx-stage.yaml"
  return self
end
...
function NginxStage:default()
  ...
  assert(fout, string.format("%s: Failed to open nginx_stage configuration file.", self.file))
  fout:write(
    yaml.dump(
      { {
...
--- Update the `nginx_stage` configuration file _nginx-stage.yaml_.
function NginxStage:update()

end

return NginxStage:new()
